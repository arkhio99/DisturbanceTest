...
```{r}
library(dplyr)
exogen_data <- dplyr::select(finished_data, -c("IsDisturbed", "PersonalDisturbance"))
...
qual_cols <- dplyr::select(exogen_data, -quant_col_names)
...
```{r}
last_finished_data <- finished_data
finished_data <- data.frame(
  ...
  PersonalDisturbance = finished_data$PersonalDisturbance)
```
...
split <- sample.split(finished_data$PersonalDisturbance, SplitRatio=0.7)
...
```{r}
summary(train$PersonalDisturbance)
```
Как видим, количество количество "Нет" меньше, чем "Да". Выполним DownSampling:
```{r}
library(caret)
train <- upSample(train[c("pc1", "pc2", "pc3", "pc4", "pc5")], y=train$PersonalDisturbance, yname="PersonalDisturbance")
summary(train$PersonalDisturbance)
```

## Логит-модель
```{r}
model_l_c <- glm(PersonalDisturbance ~ pc1 + pc2 + pc3 + pc4 + pc5,
                 train,
...
```{r}
model_l_n <- glm(PersonalDisturbance ~ 0 + pc1 + pc2 + pc3 + pc4 + pc5,
                 train,
                 family = binomial("logit"))
summary(model_l_n)
```
...
```
Как видим, наблюдаемое хи-квадрат меньше критического, а значит гипотеза о том, что упрощённая модель лучше комплексной принимается с уровнем значимости 0.05  
```{r}
model_l <- model_l_n
```
## Пробит-модель
```{r}
model_p_c <- glm(PersonalDisturbance ~ pc1 + pc2 + pc3 + pc4 + pc5,
                 train,
...
```{r}
model_p_n <- glm(PersonalDisturbance ~ 0 + pc1 + pc2 + pc3 + pc4 + pc5,
                 train,
                 family = binomial("probit"))
summary(model_p_n)
```
...
## Гомпит-модель
```{r}
model_g_c <- glm(PersonalDisturbance ~ pc1 + pc2 + pc3 + pc4 + pc5,
                 train,
...
```{r}
model_g_n <- glm(PersonalDisturbance ~ 0 + pc1 + pc2 + pc3 + pc4 + pc5,
                 train,
                 family = binomial("cloglog"))
summary(model_g_n)
```
...
info_crits <- data.frame(AICs, BICs, LogLH, hlTests, pR2s, ks_pvalues)
rownames(info_crits) <- c("Logit", "Probit", "Gompit")
info_crits
```
Меньший AIC принадлежит гомпит-модели  
Меньший BIC принадлежит гомпит-модели  
Меньший LogLH принадлежит логит-модели  
Наибольший $R^2$ принадлежит гомпит-модели  
Исходя из критерия Хосмера-Лемешоу, все модели согласованы с выборкой с уровнем значимости 0.05. Набиольший у пробит-модели  
Исходя из p-значения для критерия Колмогорова-Смирнова, остатки распределены нормально только у пробит-модели с уровнем значимости 0.05.  
Выберем гомпит-модель  .

```{r}
model_upsample <- model_g
```
  
## Оценка построенной модели на основе графика QQ
```{r}
qqnorm(model_upsample$residuals)
qqline(model_upsample$residuals, col="steelblue", lwd=2)
```
## Построение прогноза на основе тестовой выборки при помощи логит-модели
```{r}
test$predict <- predict(model_upsample, test, type='response')
test$predict <- as.factor(ifelse(test$predict < 0.4, "low", "high"))
confusionMatrix(data=test$predict, reference=test$PersonalDisturbance, positive="high")
```
...
```{r}
library(ROCR)
pr <- prediction(predict(model_upsample, test, type='response'), as.numeric(test$PersonalDisturbance))
